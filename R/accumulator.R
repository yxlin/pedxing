...
##' The Probability of Moving Up or Down (Method 1)
##' 
##' p gives the probability (p) of moving one unit of evidence (ie dx) up. 
##' Setting is_minus to TRUE results in a probability of 1 - p, which is the 
##' probability of moving down one unit. get_dx returns the 
##' displacement of one evidence unit. These functions implement the 
##' equation (6) on page 445.
##' 
##' @param v the drift rate
##' @param s the standard deviation of a diffusion process. Note the variance
##' of the process is s^2.
##' @param h the size of  a time step 
##' @param is_minus if TRUE, return q function; otherwise, return p function. 
##' @return a probability value. vectorization depends on R's default behaviour.
##' @references
##' Tuerlinckx, F., Maris, E., Ratcliff, R., & De Boeck, P. (2001). A 
##' comparison of four methods for simulating the diffusion process. 
##' Behavior Research Methods, Instruments, & Computers, 33(4), 443-456.
##' @examples
##' mu <- 1.2
##' sigma <- 1
##' h <- .001        ## time step
...
##' 
##' n <- 1000     
##' prob_up <- p(v=mu, s=sigma, h=h)  
##' prob_down <- p(v=mu, s=sigma, h=h, is_minus = TRUE)  
##' 
##' y <- sample(x = c(dx, -dx), size = n, replace = TRUE, 
##'             prob = c(prob_up, prob_down))
##' plot(1:n, cumsum(y), type = "l", xlab = "DT (ms)", ylab = "Evidence",
##'      main = "A Random Walk Approximation")
##' points(0, 0, col = "red")
##' @export
p <- function(v, s = 1, h = .001, is_minus = FALSE) { 
  if (is_minus) {
    out <- .5 *(1 - v*sqrt(h)/s)
  } else {
    out <- .5 *(1 + v*sqrt(h)/s)
...
##' @rdname p
##' @export
get_dx <- function(s, h) { return(s*sqrt(h)) }
