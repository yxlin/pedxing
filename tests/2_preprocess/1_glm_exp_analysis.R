...
load("tests/extdata/mid-stage/precar_given_safe_exp.RData")
# load("tests/2_preprocess/data/logistic_precar.RData")
# load("tests/2_preprocess/data/logistic_rt_before.RData")

count_par <- function(x) {
    pars <- rownames(summary(x)$summary) %w/o% c(paste0("a[", 1:15, "]"), "dev", "lp__")
    cat(pars, "\n")
    length(pars)
}
...
tmp <- count_par(m0@stanfit)
tmp + 15
tmp <- count_par(m1@stanfit)
tmp + 15
tmp <- count_par(m2@stanfit)
tmp + 15
tmp <- count_par(m3@stanfit)
tmp + 15
tmp <- count_par(m4@stanfit)
tmp + 15

# summary(m0@stanfit)$summary
# summary(m2@stanfit)$summary
# summary(m3@stanfit)$summary
# summary(m4@stanfit)$summary
...
load("tests/extdata/mid-stage/safe_given_precar_exp.RData")
ms1 <- compare(m0, m1, m2, m3, m4)
load("tests/extdata/mid-stage/safe_given_postcar_exp.RData")
ms2 <- compare(m0, m1, m2, m3, m4)
load("tests/extdata/mid-stage/safe_rt_given_precar_exp.RData")
ms3 <- compare(m0, m1, m2, m3, m4)
load("tests/extdata/mid-stage/safe_rt_given_postcar_exp.RData")
ms4 <- compare(m0, m1, m2, m3, m4)
load("tests/extdata/mid-stage/unsafe_rt_given_precar_exp.RData")
...
cairo_pdf("tests/figs/glm_exp_waic.pdf")
print(p)
dev.off()
...
ns <- sum(rownames(summary(m0@stanfit)$summary) %in% paste0("a[", 1:15, "]"))
dp0 <- data.table(
    s      = rep(1:ns, each = 8),
...
save(dpred0, dpred1, dpred2, dpred3, dpred4, dpred5, dpred6, dpred7, dpred8, dpred9, dpred10, dpred11,
    file = "tests/extdata/mid-stage3_glm_exp_analysis.rda"
)
...
cairo_pdf("tests/figs/glm_exp_gof.pdf")
print(p0)
dev.off()
